...
[![Reproducibility: <15s](https://img.shields.io/badge/reproducibility-<15s-brightgreen.svg)]()
...
| Model | Intent Macro F1 | Escalation F1 | False-Auto Rate ⚠️ | ROUGE-L | LLM Judge (1-5) | Latency |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
...
* **Human-Judge Agreement Calibration**:
  * **Cohen's Kappa ($\kappa$)**: `0.6271` (*Substantial Agreement*, >0.60 standard)
  * **Pearson Correlation ($r$)**: `0.9284` (*Strong Linear Alignment*)
  * **Adjacent Score Match**: `100.0%` (within $\pm 1$ point on 1–5 scale)
...
- [x] **3. Evaluation Harness + LLM Judge + Human Agreement Proof**: Automated metrics + 4-dimensional rubric in `docs/judge_rubric.md` + empirical Cohen's Kappa ($\kappa = 0.6271$) proof in `src/human_study.py`.
- [x] **4. Comprehensive Report**: Detailed in `REPORT.md` covering Problem framing, Baselines, Top 5 Failure Modes with verbatim tweets and hypotheses, the mandatory *"What is misleading about my headline number?"* section, and Next Week plans.
